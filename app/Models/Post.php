...
class post extends Sximo  {
    ...
    public static function lists( $tag = null) {
        ...
        foreach($rows as $row) {
            ...
            if(file_exists('./uploads/campaign/'.$row->image) and $row->image !='') {
                $row->image = asset('/uploads/campaign/'.$row->image);
            } else {
    ...
    public static function read( $alias )  {
        ...
        $posts = DB::table('tb_pages')
                    ->select('tb_pages.*', 'tb_categories.name', 'tb_users.username', 'tb_users.avatar', 'wo_mushaf.id_mushaf', 'wo_mushaf.price_mushaf', DB::raw('tb_categories.alias AS category_alias')  ,\DB::raw('COUNT(tb_comments.commentID) AS comments') ,\DB::raw('SUM(tb_comments.total_mushaf) AS get_mushaf'), \DB::raw('wo_program.description_program, wo_program.implementation_program'))
                    ->leftJoin('tb_users','tb_users.id','tb_pages.userid')
                    ->leftJoin('tb_comments','tb_comments.pageID','tb_pages.pageID')   
					->leftJoin('wo_program','wo_program.id_program','tb_pages.id_program')
					->leftJoin('wo_mushaf','wo_mushaf.id_mushaf','tb_pages.id_mushaf')					
                    ->leftJoin('tb_categories','tb_categories.cid','tb_pages.cid')                  
                    ->where('pagetype','post');
    ...
    public static function domicile( ) {

        return $domicile = DB::table('wo_region_city')
                            ->select('wo_region_city.*')
                            ->get();
    }
